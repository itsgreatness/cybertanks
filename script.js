(function () {
    ...
    async function pointToATank() {
        ...
        for (let i = 0; i < TARGETS && !TERMINATE; i++) {
            ...
            entities.push(createBullet(tanks[0], 1));
            const t = Date.now();
            while (Date.now() - startTime < 1000 && !TERMINATE) {
                tanks[0].rot = directionToPoint(tanks[0].pos[0], tanks[0].pos[2], target.pos[0], target.pos[2]);
            }
        }
    }
